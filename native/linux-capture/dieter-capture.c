...
#include "dieter-input-protocol.h"
#include <errno.h>
#include <fcntl.h>
#include <gio/gio.h>
...
#define HELPER_VERSION "linux-native-v" G_STRINGIFY(DIETER_INPUT_PROTOCOL_VERSION)
...
#define INT_MEMBER(name, value) do { json_builder_set_member_name(builder, name); json_builder_add_int_value(builder, value); } while (0)
	...
	INT_MEMBER("input_protocol_version", DIETER_INPUT_PROTOCOL_VERSION);
...
static void emit_event(CaptureApp *app, guint64 stream_id, guint64 ack, const gchar *error,
    ...
    JsonBuilder *builder = json_builder_new();
    json_builder_begin_object(builder);
	json_builder_set_member_name(builder, "version"); json_builder_add_int_value(builder, DIETER_INPUT_PROTOCOL_VERSION);
...
static gboolean handle_command(CaptureApp *app, JsonObject *command) {
    ...
    GError *error = NULL;
	if (version != DIETER_INPUT_PROTOCOL_VERSION || id == 0 || strlen(kind) > 32) {
