 ...
 * their custom VideoSinks. Except for receive-only SurfaceBuffer, implementations implement toI420(),
...
public class VideoFrame implements RefCounted {
  ...
  public interface Buffer extends RefCounted {
     ...
     * conversion will take place. Except for receive-only SurfaceBuffer, implementations provide I420 for
  ...
  /** Direct decoder storage, valid only for a surface-aware receive sink.
   *
   * Unlike convertible image buffers, a SurfaceBuffer owns a real opaque codec
   * output index until render/discard, or until its decoder generation ends.
   * render() transfers the pixels to the decoder's configured Surface exactly
   * once, without a future presentation queue. A false result means the output
   * was already consumed or invalidated; it must not count as presentation.
   * Native decode completion/statistics still use the normal retained buffer.
   * This receive-only buffer cannot be forwarded to encoders or generic sinks;
   * toI420() returns null. Select texture output when pixel conversion is needed.
   */
  public interface SurfaceBuffer extends Buffer {
    boolean render();
    void discard();
  }
