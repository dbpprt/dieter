...
class AndroidVideoDecoder implements VideoDecoder, VideoSink {
  ...
  @Nullable private final DecoderSurface directSurface;
  @Nullable private MediaCodecSurfaceFrame.Owner directOutputs;
  private java.util.function.LongConsumer decodedOutput = timestampNs -> {};
  // Real MediaCodec dequeue, before a busy texture consumer may discard output.
  // A completion carries media identity only; it is not a presentation event.
  void setDecodedOutputCallback(java.util.function.LongConsumer callback) {
    decodedOutput = callback;
  }
  private final Runnable stopForSurface = () -> release();
  ...
  AndroidVideoDecoder(MediaCodecWrapperFactory mediaCodecWrapperFactory, String codecName,
    ...
    this(mediaCodecWrapperFactory, codecName, codecType, colorFormat, sharedContext, null);
  }

  AndroidVideoDecoder(MediaCodecWrapperFactory mediaCodecWrapperFactory, String codecName,
      VideoCodecMimeType codecType, int colorFormat, @Nullable EglBase.Context sharedContext,
      @Nullable DecoderSurface directSurface) {
  ...
  @Override
  public synchronized VideoCodecStatus initDecode(Settings settings, Callback callback) {
    // A timed-out output worker still owns its codec and target. Do not mutate
    // either callback/surface or allocate a second texture helper underneath it.
    if (outputThread != null) return VideoCodecStatus.FALLBACK_SOFTWARE;
    this.decoderThreadChecker = new ThreadChecker();

    this.callback = callback;
    if (directSurface != null && directSurface.isOpen()) {
      VideoCodecStatus status = directSurface.configure(stopForSurface, output -> {
        surface = output;
        return initDecodeInternal(settings.width, settings.height);
      });
      if (status == VideoCodecStatus.OK) return status;
      directSurface.detach(stopForSurface);
      surface = null;
      // Optional direct output failed before admission. A fresh ordinary
      // texture decoder is the bounded fallback, including missing callbacks.
      this.callback = callback;
    }
  ...
  private VideoCodecStatus initDecodeInternal(int width, int height) {
    ...
    try {
      ...
      codec.start();
      if (directSurface != null && surfaceTextureHelper == null && surface != null) {
        directOutputs = new MediaCodecSurfaceFrame.Owner(codec, directSurface);
      }
    } catch (RuntimeException e) {
      Logging.e(TAG, "initDecode failed", e);
      // No output thread exists yet; releaseInternal cannot own this allocation.
  ...
  @Override
  public synchronized VideoCodecStatus decode(EncodedImage frame, DecodeInfo info) {
    decoderThreadChecker.checkIsOnValidThread();
    if (!running || codec == null || callback == null) {
  ...
  @Override
  public synchronized VideoCodecStatus release() {
    ...
    if (status == VideoCodecStatus.TIMEOUT && directSurface != null && surfaceTextureHelper == null) {
      // The worker fences/dequeues late output and detaches only after codec
      // release. Keep its borrowed target reserved while it is still alive.
      return status;
    }
    ...
    surface = null;
    if (directSurface != null) directSurface.detach(stopForSurface);
  ...
  private VideoCodecStatus releaseInternal() {
    ...
    try {
      ...
      // A timed-out worker still owns this codec. Retain its state until it
      // finishes; clearing it early would turn delayed shutdown into a use
      // after release/null dereference and permit an unsafe reinitialization.
      if (joined) {
        codec = null;
        outputThread = null;
        frameInfos.clear();
  ...
  private VideoCodecStatus reinitDecode(int newWidth, int newHeight) {
    ...
    if (directSurface != null && surfaceTextureHelper == null) {
      return directSurface.configure(stopForSurface, output -> {
        surface = output;
  ...
  protected void deliverDecodedFrame() {
    ...
    try {
      ...
      decodedOutput.accept(info.presentationTimeUs * 1000);

      if (directOutputs != null) {
        final int frameWidth;
        final int frameHeight;
        synchronized (dimensionLock) { frameWidth = width; frameHeight = height; }
        MediaCodecSurfaceFrame buffer = directOutputs.acquire(index, frameWidth, frameHeight, info.presentationTimeUs);
        if (buffer != null) {
          VideoFrame frame = new VideoFrame(buffer, rotation, info.presentationTimeUs * 1000);
          try { callback.onDecodedFrame(frame, decodeTimeMs, null); }
  ...
  private void reformat(MediaFormat format) {
    ...
    // Both texture and direct-surface mode use opaque codec storage. Validate
    // CPU-readable color formats only for actual byte-buffer output.
    if (surface == null && format.containsKey(MediaFormat.KEY_COLOR_FORMAT)) {
  ...
  private void releaseCodecOnOutputThread() {
    ...
    boolean wasDirect = directOutputs != null;
    if (wasDirect) {
      try { directOutputs.close(); }
      catch (RuntimeException e) { Logging.e(TAG, "Surface outputs release failed", e); }
      directOutputs = null;
    }
    try {
      codec.stop();
    ...
    if (wasDirect) directSurface.detach(stopForSurface);
