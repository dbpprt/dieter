...
daemon exposes native gRPC on its authenticated listeners. Native macOS, iOS, and
Android clients use the same contract locally, over an authenticated direct TLS
route, a data-only WebRTC route, or through the gateway relay. There is no parallel REST API, JSON view
