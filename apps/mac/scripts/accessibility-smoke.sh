...
if ! lsof -nP -iTCP:4242 -sTCP:LISTEN >/dev/null 2>&1; then
    ...
    while ! lsof -nP -iTCP:4242 -sTCP:LISTEN >/dev/null 2>&1; do
        sleep 1
        COUNT=$((COUNT + 1))
        if [ "$COUNT" -ge 30 ]; then echo "dieter serve did not become ready" >&2; exit 1; fi
    done
fi
trap 'pkill -x DieterMac 2>/dev/null || true; if [ -n "${SERVER_PID:-}" ]; then kill "$SERVER_PID" 2>/dev/null || true; fi' EXIT INT TERM
...
COUNT=0
while ! osascript -e 'tell application "System Events" to tell process "Dieter" to exists window 1' 2>/dev/null | grep -q true; do
    sleep 1
    COUNT=$((COUNT + 1))
    if [ "$COUNT" -ge 30 ]; then echo "Dieter window did not appear" >&2; exit 1; fi
done
sleep 2

POSITION=$(osascript -e 'tell application "System Events" to tell process "Dieter" to get position of window 1' | tr ',' ' ')
set -- $POSITION
WINDOW_X=$1
WINDOW_Y=$2
...
on run argv
    ...
    tell application "Dieter" to activate
    delay 0.2
