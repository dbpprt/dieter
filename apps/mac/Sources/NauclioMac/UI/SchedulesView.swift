...
struct SchedulesView: View {
    ...
    @State private var editing: Nauclio_V1_Schedule?

    var body: some View {
        HSplitView {
            VStack(spacing: 0) {
                FluidPaneChrome(background: NauclioTheme.sidebar) {
                    HStack(spacing: 8) {
                        PaneTitleBlock(
                            title: "Schedules",
                        ...
                        Button { Task { await store.loadSchedules() } } label: { Image(systemName: "arrow.clockwise") }.buttonStyle(NauclioIconButtonStyle())
                        Button { editing = nil; editorPresented = true } label: { Label("New", systemImage: "plus") }
                            .buttonStyle(NauclioPrimaryButtonStyle()).accessibilityIdentifier("schedules.new")
                    }
                }
            ...
            }.frame(minWidth: 280, idealWidth: 350, maxWidth: 440, maxHeight: .infinity, alignment: .top).background(NauclioTheme.sidebar)
...
struct ScheduleRow: View {
    let schedule: Nauclio_V1_Schedule
    let selected: Bool
        ...
        VStack(spacing: 0) {
            FluidPaneChrome(background: NauclioTheme.sidebar, spacing: 8) {
                HStack(spacing: 10) {
                    ...
                    Button("Edit", action: edit).buttonStyle(NauclioSecondaryButtonStyle())
                    Button("Run now") { Task { await store.runSchedule(schedule) } }.buttonStyle(NauclioPrimaryButtonStyle())
                }
            } secondary: {
                HStack(spacing: 8) {
            ...
            ScrollView {
                ...
                ForEach(store.scheduleRuns, id: \.id) { run in
                    HStack { StatusPill(text: run.status, color: runtimeColor(run.status)); VStack(alignment: .leading) { Text(run.scheduledFor); if !run.message.isEmpty { Text(run.message).font(.caption).foregroundStyle(.secondary) } }; Spacer(); Text(run.manual ? "Manual" : "Scheduled").font(.caption).foregroundStyle(.secondary); if !run.cardID.isEmpty { Button("Open card") { store.section = .board; Task { await store.openConversation(cardID: run.cardID) } } } }
                        .padding(10).nauclioSurface(radius: 8)
                }
                }.padding(20)
            }
        }
...
struct ScheduleEditor: View {
    ...
    let schedule: Nauclio_V1_Schedule?
    @State private var draft: Nauclio_V1_ScheduleDraft
    @State private var preview: [String] = []

    init(schedule: Nauclio_V1_Schedule?) {
        self.schedule = schedule
        var draft = Nauclio_V1_ScheduleDraft()
    ...
    private func previewRuns() async {
        ...
        var request = Nauclio_V1_PreviewScheduleRequest(); request.cron = draft.cron; request.timezone = draft.timezone; request.count = 5
