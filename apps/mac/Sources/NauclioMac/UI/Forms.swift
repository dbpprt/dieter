...
struct NewConversationSheet: View {
    ...
    private var selectedLane: Nauclio_V1_Lane? { store.selectedBoard?.lanes.first { $0.id == lane } }
        ...
        VStack(spacing: 0) {
            HStack(alignment: .top, spacing: 12) {
                ...
                Button { dismiss() } label: { Image(systemName: "xmark").font(.system(size: 12, weight: .bold)) }
                    .buttonStyle(NauclioIconButtonStyle()).help("Close")
            }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 17) {
                    ...
                    TextField("What should this agent accomplish?", text: $title)
                        ...
                        .overlay(RoundedRectangle(cornerRadius: 10).stroke(focusedField == .title ? NauclioTheme.cobalt.opacity(0.85) : NauclioTheme.strongBorder, lineWidth: focusedField == .title ? 2 : 1))
                    ...
                    if let labels = store.selectedBoard?.labels, !labels.isEmpty {
                        newCardLabel("Labels")
                        NauclioFlowLayout(horizontalSpacing: 10, verticalSpacing: 8) {
                            ForEach(labels, id: \.id) { label in
                                let selected = selectedLabelIDs.contains(label.id)
                                let tint = Color(hex: label.color) ?? NauclioTheme.aegean
                                Button {
                                    if selected { selectedLabelIDs.remove(label.id) } else { selectedLabelIDs.insert(label.id) }
                                } label: {
                                    HStack(spacing: 7) {
                                        Image(systemName: selected ? "checkmark.square.fill" : "square")
                                            .foregroundStyle(selected ? tint : NauclioTheme.tertiary)
                    ...
                    HStack(spacing: 10) {
                        Image(systemName: "lock").foregroundStyle(NauclioTheme.aegean)
                        Text("Nauclio persists one local harness session and transcript for this card on \(store.endpoint.name).")
                            .font(.caption).foregroundStyle(NauclioTheme.aegean)
                        Spacer()
                    }
                    .padding(.horizontal, 13).frame(minHeight: 42)
                    .background(NauclioTheme.cobalt.opacity(0.09), in: RoundedRectangle(cornerRadius: 9))
                    .overlay(RoundedRectangle(cornerRadius: 9).stroke(NauclioTheme.cobalt.opacity(0.28)))
                }
            ...
            HStack(spacing: 10) {
                Spacer()
                Button("Cancel") { dismiss() }.buttonStyle(NauclioSecondaryButtonStyle())
                Button {
                    Task { await submit() }
                } label: {
                    HStack(spacing: 7) {
                ...
                .buttonStyle(NauclioPrimaryButtonStyle()).disabled(!canSubmit)
    ...
    private func newCardLabel(_ title: String) -> some View {
        Text(title).font(.system(size: 12, weight: .semibold)).foregroundStyle(NauclioTheme.subtle)
            .padding(.bottom, -10)
    }
    ...
    private func newCardMenu<Content: View>(
        ...
        VStack(alignment: .leading, spacing: 7) {
            Text(title).font(.system(size: 11, weight: .semibold)).foregroundStyle(NauclioTheme.subtle)
            Menu(content: content) {
                HStack(spacing: 7) {
                    Image(systemName: symbol).font(.system(size: 10, weight: .semibold)).foregroundStyle(NauclioTheme.aegean)
                    Text(value).lineLimit(1)
                    Spacer(minLength: 4)
                    Image(systemName: "chevron.down").font(.system(size: 7, weight: .bold)).foregroundStyle(NauclioTheme.tertiary)
                }
                .font(.system(size: 11, weight: .medium)).foregroundStyle(NauclioTheme.subtle)
                .padding(.horizontal, 10).frame(height: 38)
                .background(NauclioTheme.input, in: RoundedRectangle(cornerRadius: 8))
                .overlay(RoundedRectangle(cornerRadius: 8).stroke(NauclioTheme.strongBorder))
            }
...
struct ProviderOptionFields: View {
    let options: [Nauclio_V1_ProviderOption]
    @Binding var values: [String: String]

    var body: some View {
        ForEach(options, id: \Nauclio_V1_ProviderOption.id) { option in
...
private struct ProviderOptionField: View {
    let option: Nauclio_V1_ProviderOption
    @Binding var values: [String: String]
    ...
    @ViewBuilder var body: some View {
        if ["boolean", "bool"].contains(option.type.lowercased()) {
            ...
            Picker(option.name, selection: value) {
                ForEach(option.choices, id: \Nauclio_V1_ProviderOptionChoice.value) { choice in
    ...
    @ViewBuilder var body: some View {
        if ["boolean", "bool"].contains(option.type.lowercased()) {
            ...
            Menu {
                ...
                NauclioChipLabel(title: option.choices.first(where: { $0.value == currentValue })?.name ?? option.name, symbol: "slider.horizontal.3")
...
struct NewProjectSheet: View {
    ...
    private var availableMachines: [NauclioEndpoint] {
        store.machines.isEmpty ? [store.endpoint] : store.machines
    }

    private var selectedMachine: NauclioEndpoint? {
    ...
    var body: some View {
        VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 17) {
                    ...
                    Menu {
                        ...
                        HStack(spacing: 9) {
                            ...
                            Spacer()
                            Image(systemName: "chevron.down").font(.system(size: 8, weight: .bold)).foregroundStyle(NauclioTheme.tertiary)
                        }
                        .foregroundStyle(NauclioTheme.subtle).padding(.horizontal, 13).frame(height: 48)
                    ...
                    HStack(alignment: .top, spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 7) {
                            ...
                            Menu {
                                ...
                                .font(.system(size: 13, weight: .medium)).foregroundStyle(NauclioTheme.subtle)
            ...
            HStack(spacing: 10) {
                ...
                .buttonStyle(NauclioPrimaryButtonStyle())
    ...
    private func projectLabel(_ value: String) -> some View {
        Text(value).font(.system(size: 12, weight: .semibold)).foregroundStyle(NauclioTheme.subtle)
    }
...
private struct RemoteDirectoryBrowserSheet: View {
    ...
    @State private var listing: Nauclio_V1_DirectoryListing?
    @State private var pathField = ""
    @State private var loading = false
    @State private var showHidden = false
    @State private var errorMessage = ""

    private var machine: NauclioEndpoint? { store.machines.first { $0.id == machineID } ?? (store.endpoint.id == machineID ? store.endpoint : nil) }
    private var entries: [Nauclio_V1_DirectoryEntry] {
        ...
        VStack(spacing: 0) {
            HStack(spacing: 11) {
                Image(systemName: "externaldrive.connected.to.line.below")
                    .font(.system(size: 18, weight: .semibold)).foregroundStyle(NauclioTheme.aegean)
                    .frame(width: 36, height: 36).background(NauclioTheme.elevated, in: RoundedRectangle(cornerRadius: 9))
            ...
            HStack(spacing: 0) {
                VStack(alignment: .leading, spacing: 5) {
                    ...
                    ForEach(listing?.locations ?? [], id: \.path) { location in
                        Button { Task { await load(location.path) } } label: {
                            ...
                            .font(.system(size: 12, weight: .medium)).foregroundStyle(NauclioTheme.subtle)
                            .padding(.horizontal, 9).frame(height: 30)
                            .background(listing?.path == location.path ? NauclioTheme.raised : .clear, in: RoundedRectangle(cornerRadius: 7))
                        }.buttonStyle(.plain)
                    }
                    Spacer()
                    Toggle("Show hidden folders", isOn: $showHidden).toggleStyle(.checkbox).font(.caption).foregroundStyle(NauclioTheme.subtle)
                }
                .padding(12).frame(width: 165).background(NauclioTheme.sidebar)
                ...
                VStack(spacing: 10) {
                    HStack(spacing: 7) {
                        ...
                        .buttonStyle(NauclioIconButtonStyle()).disabled(listing?.parent.isEmpty != false || loading)
                        TextField("Path on \(machine?.name ?? "machine")", text: $pathField)
                            .textFieldStyle(.plain).font(.system(size: 12, design: .monospaced))
                            .padding(.horizontal, 10).frame(height: 30)
                            .background(NauclioTheme.input, in: RoundedRectangle(cornerRadius: 7))
                            .overlay(RoundedRectangle(cornerRadius: 7).stroke(NauclioTheme.strongBorder))
                            .onSubmit { Task { await load(pathField) } }
                        Button("Go") { Task { await load(pathField) } }.buttonStyle(NauclioSecondaryButtonStyle()).disabled(loading)
                    }
                    ...
                    if !errorMessage.isEmpty {
                        ...
                        .font(.caption).foregroundStyle(NauclioTheme.coral)
                    }
                                ...
                                Button { Task { await load(entry.path) } } label: {
                                    HStack(spacing: 10) {
                                        Image(systemName: entry.gitRepository ? "folder.badge.gearshape" : "folder.fill")
                                            .foregroundStyle(entry.gitRepository ? NauclioTheme.seafoam : NauclioTheme.aegean)
                                            .frame(width: 19)
                                        Text(entry.name).font(.system(size: 12, weight: .medium)).lineLimit(1)
                                        Spacer()
                                        if entry.gitRepository {
                                            Text("Git repository").font(.caption2.weight(.semibold)).foregroundStyle(NauclioTheme.seafoam)
                                                .padding(.horizontal, 7).padding(.vertical, 3).background(NauclioTheme.seafoam.opacity(0.1), in: Capsule())
                                        }
                                        Image(systemName: "chevron.right").font(.system(size: 8, weight: .bold)).foregroundStyle(NauclioTheme.tertiary)
                                    }
                                    .foregroundStyle(NauclioTheme.subtle).padding(.horizontal, 11).frame(height: 36)
            ...
            HStack {
                if listing?.gitRepository == true {
                    Label("\(listing?.name ?? "Folder") is a Git working tree", systemImage: "checkmark.circle.fill")
                        .font(.caption).foregroundStyle(NauclioTheme.seafoam)
                } else {
                    Text("Open a folder containing a .git working tree to select it.").font(.caption).foregroundStyle(NauclioTheme.tertiary)
                }
                Spacer()
                Button("Cancel") { dismiss() }.buttonStyle(NauclioSecondaryButtonStyle())
                ...
                .buttonStyle(NauclioPrimaryButtonStyle()).disabled(listing?.gitRepository != true)
            }
            .padding(14)
        }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            if let project = store.selectedProject {
                Text(project.name).font(.caption.weight(.semibold)).foregroundStyle(NauclioTheme.aegean)
            }
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("Persistent context") { TextEditor(text: $prompt).font(.body.monospaced()).frame(height: 260); Text("This context is owned by Nauclio and supplied to new work without writing into the repository.").font(.caption).foregroundStyle(.secondary) }
