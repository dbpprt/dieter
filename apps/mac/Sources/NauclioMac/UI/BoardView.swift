...
enum BoardDropOrdering {
    static func position(before targetCardID: String, movingCardID: String, cards: [Nauclio_V1_Card]) -> Int64? {
...
private struct ConversationResizeDivider: View {
    ...
    var body: some View {
        ZStack {
            ...
            Rectangle()
                .fill(hovering ? NauclioTheme.aegean.opacity(0.62) : NauclioTheme.border)
...
struct BoardView: View {
    ...
    @AppStorage("nauclio.conversationPaneWidth") private var conversationPaneWidth = Double(ConversationPaneSizing.defaultWidth)
...
struct BoardHeader: View {
    ...
    var body: some View {
        ...
        FluidPaneChrome(background: NauclioTheme.background, spacing: 7) {
            HStack(spacing: 10) {
                    ...
                    HStack(spacing: 7) {
                        ...
                        Menu {
                            ...
                            Image(systemName: "chevron.down")
                                .font(.system(size: 8, weight: .bold)).foregroundStyle(NauclioTheme.tertiary)
                ...
                Button { store.projectContextPresented = true } label: { Image(systemName: "ellipsis") }
                    .buttonStyle(NauclioIconButtonStyle()).help("Project context")
            }
        } secondary: {
            ViewThatFits(in: .horizontal) {
                ...
                } label: { NauclioChipLabel(title: store.runtimeFilter.isEmpty ? "All states" : store.runtimeFilter.capitalized) }
                    .menuStyle(.borderlessButton).menuIndicator(.hidden).fixedSize()

                Button { store.archivePolicyPresented = true } label: {
                    NauclioChipLabel(title: retentionTitle, symbol: "archivebox")
                }
                .buttonStyle(.plain)
                ...
                Button { store.labelsPresented = true } label: {
                    ...
                    .font(.caption2.weight(.medium)).foregroundStyle(NauclioTheme.subtle)
...
private struct BoardLabelShelfChip: View {
    ...
    private var color: Color { Color(hex: label.color) ?? NauclioTheme.aegean }
    ...
    var body: some View {
        Button(action: select) {
            HStack(spacing: 6) {
                ...
                Text("· \(count)").foregroundStyle(NauclioTheme.tertiary)
            }
            .font(.system(size: 11, weight: .semibold))
            .foregroundStyle(selected ? color : NauclioTheme.subtle)
            .padding(.horizontal, 10).frame(height: 28)
...
private struct BoardLabelDragPreview: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text("Drop onto a card").font(.system(size: 10)).foregroundStyle(NauclioTheme.tertiary)
        }
...
struct KanbanView: View {
    @Environment(NauclioStore.self) private var store
    let board: Nauclio_V1_Board

    private var lanes: [Nauclio_V1_Lane] {
        if !board.lanes.isEmpty { return board.lanes }
        return ["backlog", "ready", "running", "review", "done"].map { id in
            var lane = Nauclio_V1_Lane(); lane.id = id; lane.name = id.capitalized; return lane
        }
    }
                ...
                LazyVStack(spacing: 0) {
                    ...
                    if cards.isEmpty {
                        VStack(spacing: 7) {
                            ...
                            .font(.caption).foregroundStyle(isDropTargeted ? NauclioTheme.aegean : NauclioTheme.tertiary)
                            .frame(maxWidth: .infinity).padding(.vertical, 28)
                            .overlay(RoundedRectangle(cornerRadius: 9).stroke(isDropTargeted ? NauclioTheme.aegean.opacity(0.55) : NauclioTheme.border, style: .init(dash: [5])))
                    }
                }
            }
        }
...
private struct BoardCardDragPreview: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            Image(systemName: "rectangle.on.rectangle.angled").foregroundStyle(NauclioTheme.aegean)
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack(spacing: 6) {
                    ...
                    Text(card.runtime.capitalized).font(.system(size: 9, weight: .medium)).foregroundStyle(NauclioTheme.tertiary)
                }
            }
            Spacer(minLength: 8)
        }
        .padding(12).frame(width: 240)
        .background(NauclioTheme.elevated, in: RoundedRectangle(cornerRadius: 10, style: .continuous))
        .overlay(RoundedRectangle(cornerRadius: 10, style: .continuous).stroke(NauclioTheme.aegean.opacity(0.4)))
...
struct BoardCardView: View {
    ...
    var labels: [Nauclio_V1_Label] { store.selectedBoard?.labels.filter { card.labelIds.contains($0.id) } ?? [] }
            ...
            .overlay(alignment: .topTrailing) {
                if labelDropTargeted {
                    Image(systemName: "tag.fill")
                        .font(.system(size: 10, weight: .bold)).foregroundStyle(NauclioTheme.seafoam)
                        .padding(7)
                        .background(NauclioTheme.background.opacity(0.9), in: Circle())
                        .padding(5)
