...
struct TerminalsView: View {
    ...
    @State private var closeCandidate: Nauclio_V1_Terminal?
    @State private var renamePresented = false
    @State private var renameValue = ""

    private var selected: Nauclio_V1_Terminal? { store.selectedTerminal }
        ...
        VStack(spacing: 0) {
            FluidPaneChrome {
                HStack(spacing: 12) {
                    ...
                    .background(NauclioTheme.cobalt, in: RoundedRectangle(cornerRadius: 7, style: .continuous))
    ...
    private func terminalWorkspace(_ terminal: Nauclio_V1_Terminal) -> some View {
        VStack(spacing: 0) {
    ...
    private func terminalStatusColor(_ terminal: Nauclio_V1_Terminal) -> SwiftUI.Color {
        if terminal.status != "running" { return terminal.hasExitCode && terminal.exitCode == 0 ? NauclioTheme.tertiary : NauclioTheme.coral }
        return store.terminalStreamConnected ? NauclioTheme.seafoam : NauclioTheme.amber
    }

    private func terminalStatusText(_ terminal: Nauclio_V1_Terminal) -> String {
...
private struct TerminalTab: View {
    let terminal: Nauclio_V1_Terminal
    let selected: Bool
    let select: () -> Void
    let close: () -> Void
    ...
    var body: some View {
        HStack(spacing: 7) {
            Button(action: select) {
                HStack(spacing: 7) {
                    Circle()
                        .fill(terminal.status == "running" ? NauclioTheme.seafoam : NauclioTheme.tertiary)
            ...
            Button(action: close) {
                Image(systemName: "xmark")
                    ...
                    .background(hovering ? NauclioTheme.raised : Color.clear, in: RoundedRectangle(cornerRadius: 4))
            }
            .buttonStyle(.plain)
            .foregroundStyle(NauclioTheme.tertiary)
            .help("Close terminal")
        ...
        .padding(.trailing, 7)
        .frame(height: 38)
        .background(selected ? NauclioTheme.background : Color.clear)
        .overlay(alignment: .bottom) {
            Rectangle().fill(selected ? NauclioTheme.aegean : Color.clear).frame(height: 1)
        }
        .overlay(alignment: .trailing) { Rectangle().fill(NauclioTheme.border).frame(width: 1) }
        .onHover { hovering = $0 }
        .contextMenu {
...
private struct NewTerminalSheet: View {
    ...
    private var availableProjects: [Nauclio_V1_Project] { store.projects.filter { !$0.archived } }
    private var selectedProject: Nauclio_V1_Project? { availableProjects.first { $0.id == projectID } }
    ...
    var body: some View {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("Runs persistently on \(store.endpoint.name)")
                        .font(NauclioFont.meta).foregroundStyle(NauclioTheme.tertiary)
                }
                Spacer()
            }
            .padding(18)
        ...
        .background(NauclioTheme.surface)
        .onAppear {
