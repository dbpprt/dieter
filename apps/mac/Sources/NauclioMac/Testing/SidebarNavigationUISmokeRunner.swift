...
enum SidebarNavigationUISmokeRunner {
    ...
    private static func prepare(window: NSWindow, results: inout [String: String]) async {
        ...
        try? await NauclioTaskSleep.milliseconds(450)
    ...
    private static func seed(_ store: NauclioStore) {
        let names = ["Alpha", "Beta", "Gamma"]
        var projects: [Nauclio_V1_Project] = []
        var boardsByProject: [String: [Nauclio_V1_Board]] = [:]
    ...
    private static func drag(window: NSWindow, fromX: CGFloat, fromTop: CGFloat, toX: CGFloat, toTop: CGFloat) async {
        ...
        for step in 1...30 {
            ...
            try? await NauclioTaskSleep.milliseconds(35)
        }
        try? await NauclioTaskSleep.milliseconds(550)
    ...
    private static func outputDirectory() -> URL {
        ...
        return URL(filePath: NSTemporaryDirectory()).appending(path: "nauclio-sidebar-ui-smoke", directoryHint: .isDirectory)
    }
