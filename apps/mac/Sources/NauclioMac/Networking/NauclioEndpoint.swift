...
struct NauclioEndpoint: Codable, Equatable, Hashable, Identifiable, Sendable {
    ...
    var gatewayEndpoint: NauclioEndpoint {
        guard daemonID != nil else { return self }
        return NauclioEndpoint(name: "Nauclio Gateway", host: host, port: port, secure: secure)
    }

    static let defaults = [
        NauclioEndpoint(name: "Nauclio Gateway", host: "board.dbpprt.com", port: 443, secure: true),
    ]

    static func parse(_ value: String, name: String = "Custom") -> NauclioEndpoint? {
        ...
        if trimmed.hasPrefix("[") {
            ...
            return NauclioEndpoint(name: name, host: host, port: port, secure: secure)
        }
        ...
        return NauclioEndpoint(name: name, host: trimmed, port: secure ? 443 : 4242, secure: secure)
    }
