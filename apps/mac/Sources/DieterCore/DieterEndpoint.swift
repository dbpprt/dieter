...
package struct DieterEndpoint: Codable, Equatable, Hashable, Identifiable, Sendable {
    ...
    package var credentialID: String { "\(secure ? "https" : "http")://\(host):\(port)" }
    ...
    package var address: String { "\(secure ? "https" : "http")://\(host):\(port)" }
    package var gatewayEndpoint: DieterEndpoint {
    ...
    package static func parse(_ value: String, name: String = "Custom") -> DieterEndpoint? {
        ...
        for prefix in ["grpcs://", "https://", "grpc://", "http://"] where trimmed.lowercased().hasPrefix(prefix) {
        ...
        if let colon = trimmed.lastIndex(of: ":") {
            ...
            guard let port = Int(trimmed[trimmed.index(after: colon)...]), !host.isEmpty, (1...65_535).contains(port)
            else { return nil }
            return DieterEndpoint(name: name, host: host, port: port, secure: secure)
        }
    ...
    package init(from decoder: Decoder) throws {
        ...
        name = try values.decode(String.self, forKey: .name); host = try values.decode(String.self, forKey: .host);
...
package enum ConnectionPhase: Equatable, Sendable {
    ...
    package var needsConnectionOverlay: Bool {
        self == .authenticationRequired
    }
}

package enum MachinePresenceText {
    package static let offlineAfter: TimeInterval = 30

    package static func online(serverOnline: Bool, lastSeenAt: String, relativeTo now: Date = Date()) -> Bool {
        guard serverOnline else { return false }
        guard parse(lastSeenAt) != nil else { return true }
        return isFresh(lastSeenAt, relativeTo: now)
    }

    package static func isFresh(_ value: String, relativeTo now: Date = Date()) -> Bool {
