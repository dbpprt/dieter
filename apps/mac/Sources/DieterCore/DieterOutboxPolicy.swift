...
package enum DieterConversationOpenFailureDisposition: Equatable {
    case ignore
    case retry
    case report
}
...
package enum DieterOutboxPolicy {
    ...
    package static func synchronizedConversationID(
    ...
    package static func removeUndelivered(
    ...
    package static func nextRetryDelay(
        in entries: [DieterOutboxEntry],
        endpointID: String,
        now: Date = Date()
    ) -> TimeInterval? {
