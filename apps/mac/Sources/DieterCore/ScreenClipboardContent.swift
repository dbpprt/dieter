    ...
    public struct ScreenClipboardContent: Sendable {
        ...
        public func validate() throws {
            guard (text?.utf8.count ?? 0) <= 1024 * 1024, items.count <= 64,
                ...
                text == nil || items.isEmpty
            else { throw Self.error("Clipboard limit: 1 MiB text or 8 MiB across 64 files") }
            ...
            for item in items {
                guard !item.name.isEmpty, item.name.utf8.count <= 255, ![".", ".."].contains(item.name),
                    ...
                    names.insert(item.name.precomposedStringWithCanonicalMapping.lowercased()).inserted,
                    item.mimeType.utf8.count <= 128,
                    item.kind == 0
                        || (item.kind == 1 && items.count == 1
                            && ["image/png", "image/jpeg", "image/tiff", "image/webp"].contains(item.mimeType))
            ...
            if board.availableType(from: [.fileURL]) != nil {
                ...
                for url in urls {
                    ...
                    guard values.isRegularFile == true, values.isSymbolicLink != true else {
                        throw error("Copy regular files; folders and symbolic links are not supported")
                    }
                    guard let size = values.fileSize, size <= binaryLimit - total else {
                        throw error("Clipboard files exceed 8 MiB")
                    }
                    ...
                    var data = Data()
                    do {
                        while let part = try handle.read(upToCount: min(65536, binaryLimit - total - data.count + 1)),
                            !part.isEmpty
                        {
                            data.append(part)
                    ...
                    items.append(
                        .init(
                            kind: 0, name: url.lastPathComponent,
                            mimeType: UTType(filenameExtension: url.pathExtension)?.preferredMIMEType
                                ?? "application/octet-stream", data: data))
                }
                let value = Self(items: items); try value.validate(); return value
            }
            if let type = board.availableType(from: [
                .png, .tiff, NSPasteboard.PasteboardType("public.jpeg"),
                NSPasteboard.PasteboardType("org.webmproject.webp"),
            ]) {
                guard binary else { return Self() }
                guard let data = board.data(forType: type), data.count <= binaryLimit else {
                    throw error("Clipboard image is unavailable or exceeds 8 MiB")
                }
                let format = UTType(type.rawValue)
                let value = Self(items: [
                    .init(
                        kind: 1, name: "Clipboard.\(format?.preferredFilenameExtension ?? "png")",
                        mimeType: format?.preferredMIMEType ?? "image/png", data: data)
                ])
        ...
        public func write(_ board: NSPasteboard, directory: URL) throws {
            ...
            if let text {
                ...
                guard let type = UTType(mimeType: image.mimeType) else {
                    throw Self.error("Unsupported clipboard image")
                }
                board.clearContents()
                guard board.setData(image.data, forType: .init(type.identifier)) else {
                    throw Self.error("Clipboard image write failed")
                }
        ...
        public static var defaultDirectory: URL {
            let home =
                ProcessInfo.processInfo.environment["DIETER_HOME"].map { URL(fileURLWithPath: $0, isDirectory: true) }
        ...
        private func stage(directory: URL) throws -> [URL] {
            ...
            let existing = try fm.contentsOfDirectory(
                at: directory, includingPropertiesForKeys: [.creationDateKey, .isSymbolicLinkKey]
            )
            ...
            for (index, url) in existing.enumerated() {
                ...
                if info.isSymbolicLink != true
                    && (index < existing.count - 7 || (info.creationDate ?? .distantPast).timeIntervalSinceNow < -86400)
                {
                    try fm.removeItem(at: url)
                }
            }
            let batch = directory.appendingPathComponent(
                "transfer-\(Int(Date().timeIntervalSince1970 * 1000))-\(UUID().uuidString)", isDirectory: true)
            try fm.createDirectory(
                at: batch, withIntermediateDirectories: false, attributes: [.posixPermissions: 0o700])
            do {
