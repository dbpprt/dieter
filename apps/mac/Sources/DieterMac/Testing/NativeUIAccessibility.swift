...
@MainActor enum NativeUIAccessibility {
    struct Element {
        ...
        var text: String { ["accessibilityValue", "accessibilityLabel", "accessibilityTitle"].compactMap { value($0) as? String }.joined(separator: " ") }
    }
    ...
    static func elements(in window: NSWindow) -> [Element] {
        ...
        while let next = pending.popLast(), result.count < 20_000 {
            guard let object = next as? NSObject, visited.insert(ObjectIdentifier(object)).inserted else { continue }
    ...
    static func find(_ identifier: String, in window: NSWindow, fallbackLabel: String? = nil) -> Element? {
        ...
        if let view = NativeUISmokeTargets.frames[identifier]?.compactMap(\.view).first(where: {
            ...
            return Element(object: view, recordedFrame: targetWindow.convertToScreen(view.convert(view.bounds, to: nil)), recordedWindow: targetWindow)
        }
    ...
    @discardableResult
    static func click(_ identifier: String, in window: NSWindow, horizontalFraction: CGFloat = 0.5, fallbackLabel: String? = nil) -> Bool {
        guard let element = find(identifier, in: window, fallbackLabel: fallbackLabel) else {
            let inventory = elements(in: window).map { "\($0.identifier ?? "-") \($0.text.prefix(100))" }.joined(separator: "\n")
            try? inventory.write(to: WorkspaceUISmokeRunner.outputDirectory().appending(path: "missing-" + identifier.replacingOccurrences(of: "/", with: "_") + ".txt"), atomically: true, encoding: .utf8)
            return false
        }
        let frame = element.recordedFrame ?? element.frame
        guard frame.width > 0, frame.height > 0 else { return false }
        let window = element.recordedWindow ?? window
        let point = window.convertPoint(fromScreen: NSPoint(x: frame.minX + frame.width * horizontalFraction, y: frame.midY))
    ...
    static func scrollHorizontally(_ identifier: String, in window: NSWindow, delta: Int32) -> Bool {
        guard let scroll = horizontalScrollView(identifier, in: window),
              let cgEvent = CGEvent(scrollWheelEvent2Source: nil, units: .pixel, wheelCount: 2, wheel1: 0, wheel2: delta, wheel3: 0),
              let event = NSEvent(cgEvent: cgEvent) else { return false }
    ...
    static func arrow(down: Bool, in window: NSWindow) {
        ...
        for type in [NSEvent.EventType.keyDown, .keyUp] {
            if let event = NSEvent.keyEvent(with: type, location: .zero, modifierFlags: [], timestamp: ProcessInfo.processInfo.systemUptime,
                windowNumber: window.windowNumber, context: nil, characters: character, charactersIgnoringModifiers: character, isARepeat: false, keyCode: down ? 125 : 126) {
                NSApp.postEvent(event, atStart: false)
            }
        }
    }
