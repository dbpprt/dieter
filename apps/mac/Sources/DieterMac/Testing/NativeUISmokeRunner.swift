        ...
        static func run(store: DieterStore) async {
            ...
            do {
                ...
                if let liveCard, let machine = store.machine(forProjectID: liveCard.projectID) {
                    ...
                    let deliveryStatusVisible = await waitUntil(timeout: 5) {
                        store.conversationContext.deliveryStatus() != nil
                            && NativeUIAccessibility.find("conversation.delivery-status", in: window) != nil
                            && NativeUIAccessibility.find("machine.\(machine.daemonID ?? machine.id).queue", in: window)
                                == nil
                    }
                    results["17a-offline-message-queued"] =
                        queued && deliveryStatusVisible && store.composerText.isEmpty
                        ? "passed"
                        : "failed: queued=\(store.outboxSummary(for: machine)?.messageCount ?? 0), composer status=\(deliveryStatusVisible), draft=\(store.composerText)"
