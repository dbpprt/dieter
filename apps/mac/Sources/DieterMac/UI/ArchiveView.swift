...
struct ArchiveView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            FluidPaneChrome(background: DieterTheme.background, spacing: 9) {
                HStack(spacing: 10) {
                    PaneTitleBlock(
                        title: "Archive",
                    ...
                    .buttonStyle(DieterIconButtonStyle()).help("Refresh archive")
                }
            } secondary: {
                ...
                if scope == "Cards" {
                    ForEach(store.archivedCards, id: \.id) { card in
                        HStack {
                            VStack(alignment: .leading) {
                                ...
                                Text(card.summary).font(.caption).foregroundStyle(.secondary).lineLimit(2)
                            }; Spacer();
                            Button("Restore") {
                                Task {
                    ...
                    ForEach(store.chats.filter(\.archived), id: \.id) { card in
                        HStack {
                            VStack(alignment: .leading) {
                                Text(card.title).fontWeight(.semibold);
                                Text(card.updatedAt).font(.caption).foregroundStyle(.secondary)
                            }; Spacer();
                            Button("Restore") {
                                Task {
                                    await store.archive(card, archived: false); await store.loadArchive()
