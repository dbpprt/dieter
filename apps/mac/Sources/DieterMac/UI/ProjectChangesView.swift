...
struct ProjectChangesView: View {
    ...
    private var targetKey: String { "\(store.selectedProjectID)|\(store.endpoint.id)|\(store.connectionGeneration)|\(store.phase.isConnected)|\(scenePhase)" }
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            VStack(spacing: 0) {
                if compact {
                    if showCompactDiff, ready, model.selection != nil { diffPane(compact: true) }
                    else { fileNavigator }
                } else {
                    HSplitView {
        ...
        .task(id: targetKey) {
            ...
            guard store.projectEndpointIDs[store.selectedProjectID] == nil || store.projectEndpointIDs[store.selectedProjectID] == store.endpoint.id else { return }
        ...
        .confirmationDialog("Discard changes to \(discardPath ?? "this file")?", isPresented: Binding(
            get: { discardPath != nil }, set: { if !$0 { discardPath = nil } }
        ), presenting: discardPath) { path in
            ...
            Text("Discard staged and unstaged changes to \(path). Dieter saves a recovery copy first, then restores the file to HEAD. Untracked files are removed.")
        }
    }
    ...
    private var initialState: some View {
        Group {
            ...
            } else { ProgressView("Reading changes…") }
        ...
        VStack(spacing: 0) {
            ...
            if ready, model.changes != nil {
                ...
                HStack(spacing: 7) {
                    ...
                    TextField("Filter files", text: $filter, prompt: Text("Filter files").foregroundStyle(DieterTheme.tertiary))
                    ...
                    if !filter.isEmpty {
                        Button { filter = "" } label: { Image(systemName: "xmark.circle.fill") }
                            .buttonStyle(.plain).foregroundStyle(DieterTheme.tertiary).accessibilityLabel("Clear file filter")
                    }
                }
    ...
    private var commitComposer: some View {
        ...
        return VStack(spacing: 8) {
            VStack(alignment: .leading, spacing: 0) {
                TextField("Commit subject", text: $model.commitSubject, prompt: Text("Commit subject").foregroundStyle(DieterTheme.tertiary))
                ...
                TextField("Description (optional)", text: $model.commitBody, prompt: Text("Description (optional)").foregroundStyle(DieterTheme.tertiary), axis: .vertical)
                ...
                }.font(.system(size: 10)).foregroundStyle(DieterTheme.tertiary).padding(.horizontal, 10).padding(.bottom, 9)
            }
            ...
            Button {
                model.startOperation(kind: "commit", parameters: [
                ...
                HStack(spacing: 6) {
                    ...
                    Text(model.pendingKind == "commit" ? "Committing…" : count == 0 ? "Commit staged changes" : "Commit \(count) \(count == 1 ? "file" : "files")")
                }.frame(maxWidth: .infinity)
            }
            .buttonStyle(ChangesActionButtonStyle(prominent: true))
            .disabled(!canMutate || count == 0 || model.commitSubject.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
    ...
    private func fileSection(_ title: String, section: String, files: [Dieter_V1_ChangedFile]) -> some View {
        Section {
            ...
            if visible.isEmpty {
                Text(files.isEmpty ? (section == "staged" ? "No staged files" : "No local changes") : "No matching files")
    ...
    private func moveSelection(by offset: Int) {
        let choices = model.stagedFiles.map { ProjectChangeSelection(path: $0.path, section: "staged") }
    ...
    private func fileRow(_ file: Dieter_V1_ChangedFile, section: String) -> some View {
        ...
        return HStack(spacing: 7) {
            Button { model.startOperation(kind: stage ? "stage" : "unstage", path: file.path) } label: {
                ZStack {
                    RoundedRectangle(cornerRadius: 3).fill(stage ? .clear : DieterTheme.reviewAccent)
                    RoundedRectangle(cornerRadius: 3).stroke(stage ? DieterTheme.strongBorder : DieterTheme.reviewAccent)
                    if !stage { Image(systemName: "checkmark").font(.system(size: 9, weight: .bold)).foregroundStyle(Color.black.opacity(0.8)) }
            ...
            Button {
                ...
                HStack(spacing: 7) {
                    ...
                    Text(WorkspaceChangePresentation.filename(file.path)).font(.system(size: 12, weight: .medium)).lineLimit(1).layoutPriority(1)
                    let directory = WorkspaceChangePresentation.directory(file.path)
                    if !directory.isEmpty {
                        Text(directory).font(.system(size: 10)).foregroundStyle(DieterTheme.tertiary).lineLimit(1).truncationMode(.middle)
                    }
                    Spacer(minLength: 0)
                    if file.staged && file.unstaged {
                        Image(systemName: "circle.lefthalf.filled").font(.system(size: 10)).foregroundStyle(DieterTheme.tertiary)
                            .help("This file has both staged and unstaged edits")
                    }
                    Text(WorkspaceChangePresentation.badge(status: status, conflicted: file.conflicted, untracked: status == "untracked"))
                        .font(.system(size: 10, weight: .semibold, design: .monospaced)).foregroundStyle(statusColor(status)).frame(width: 12)
                }.frame(maxWidth: .infinity, minHeight: 32).contentShape(Rectangle())
            }
            .buttonStyle(.plain).help(file.path)
            .accessibilityLabel("\(section.capitalized) \(file.path)").accessibilityAddTraits(selected ? .isSelected : [])
            .accessibilityIdentifier("project-changes.\(section).\(file.path)").smokeTarget("project-changes.\(section).\(file.path)")
        }
        .padding(.horizontal, 6)
        .background(selected ? DieterTheme.elevated : .clear, in: RoundedRectangle(cornerRadius: 5))
        .overlay(alignment: .leading) { if selected { RoundedRectangle(cornerRadius: 1).fill(DieterTheme.reviewAccent).frame(width: 2).padding(.vertical, 6) } }
        .contextMenu {
        ...
        VStack(spacing: 0) {
            if ready, let selection = model.selection {
                ...
                if let diff = model.diff {
                    if diff.binary {
                        ContentUnavailableView("Binary file", systemImage: "doc.badge.ellipsis", description: Text("This file cannot be displayed as a text diff."))
                            .frame(maxWidth: .infinity, maxHeight: .infinity)
                    } else {
                        WorkspaceDiffContent(diff: diff, split: diffMode == "Split", comments: [], canComment: false, addComment: { _ in }, loadMore: { model.loadMore() }, loadingMore: model.diffLoading, reviewSection: selection.section)
            ...
            } else if !ready || model.changes == nil { initialState }
            else {
                ContentUnavailableView("Working tree is clean", systemImage: "checkmark.circle", description: Text("No local changes in \(store.selectedProject?.name ?? "this project")."))
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    private func diffToolbar(selection: ProjectChangeSelection, compact: Bool) -> some View {
        ...
        HStack(spacing: 10) {
            if compact {
                Button { showCompactDiff = false } label: { Image(systemName: "chevron.left") }
            ...
            Text(WorkspaceChangePresentation.filename(selection.path)).font(.system(size: 12, weight: .semibold, design: .monospaced)).lineLimit(1).truncationMode(.middle)
                .help(selection.path)
            if geometry.size.width > 900 {
                Text(WorkspaceChangePresentation.directory(selection.path))
                    .font(.system(size: 10)).foregroundStyle(DieterTheme.tertiary).lineLimit(1).truncationMode(.middle)
            }
            if geometry.size.width > 680 {
            Text(status.capitalized).font(.system(size: 10, weight: .semibold)).foregroundStyle(statusColor(status))
                .padding(.horizontal, 6).padding(.vertical, 4).background(statusColor(status).opacity(0.12), in: RoundedRectangle(cornerRadius: 4))
                .fixedSize()
            }
            Spacer(minLength: 0)
            if geometry.size.width > 680, let file = selectedFile {
                HStack(spacing: 5) {
                    Text("+\(staged ? file.stagedAdditions : file.unstagedAdditions)").foregroundStyle(DieterTheme.diffAddition)
                    Text("−\(staged ? file.stagedDeletions : file.unstagedDeletions)").foregroundStyle(DieterTheme.coral)
                }.font(.system(size: 10, design: .monospaced)).fixedSize()
            }
            HStack(spacing: 2) {
                ForEach(["Inline", "Split"], id: \.self) { mode in
                    Button { diffMode = mode } label: {
                        Text(mode).font(.system(size: 11, weight: .medium)).frame(width: 44, height: 25)
                            ...
                            .background(diffMode == mode ? DieterTheme.elevated : .clear, in: RoundedRectangle(cornerRadius: 4))
                    }.buttonStyle(.plain).accessibilityLabel("\(mode) diff").accessibilityAddTraits(diffMode == mode ? .isSelected : [])
                }
    ...
    private var feedback: some View {
        HStack(spacing: 8) {
            ...
            if let error = model.operationError ?? model.refreshError {
                ...
                Label("An agent is editing this checkout. Git actions resume when it finishes.", systemImage: "person.crop.circle.badge.clock").foregroundStyle(DieterTheme.amber)
            } else if model.busy { Text("Updating checkout state…") }
            else { Text(model.notice ?? "\(model.stagedFiles.count) staged · \(model.unstagedFiles.count) unstaged") }
            Spacer(minLength: 0)
