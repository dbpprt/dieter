...
final class DieterSystemAppearance {
    ...
    private static func read() -> ColorScheme {
        ...
        resolve(interfaceStyle: UserDefaults.standard.persistentDomain(forName: UserDefaults.globalDomain)?["AppleInterfaceStyle"] as? String)
    }
    ...
    private init() {
        ...
        themeObserver = DistributedNotificationCenter.default().addObserver(forName: NSNotification.Name("AppleInterfaceThemeChangedNotification"), object: nil, queue: .main) { [weak self] _ in
            Task { @MainActor in self?.refresh() }
        }
    }
