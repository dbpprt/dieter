...
enum TaskTokenUsagePresentation {
    static func label(_ usage: Dieter_V1_TokenUsage) -> String {
        ...
        let compact = count >= 1_000_000 ? String(format: "%.1fM", Double(count) / 1_000_000)
    ...
    static func detail(_ usage: Dieter_V1_TokenUsage) -> String {
        ...
        return "\(usage.totalTokens.formatted()) total tokens · \(usage.inputTokens.formatted()) input · \(usage.outputTokens.formatted()) output."
