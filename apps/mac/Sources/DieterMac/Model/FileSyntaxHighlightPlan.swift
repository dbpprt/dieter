...
enum FileSyntaxHighlightPlanner {
    static func build(
        ...
        MacPerformanceSignposts.measure("Syntax highlight plan", log: MacPerformanceSignposts.editor) {
            ...
            append(
                #"(?<![\w.])(?:0x[\da-fA-F]+|\d+(?:\.\d+)?)(?![\w.])"#, style: .number, source: source, range: range,
                to: &runs)
            ...
            case .yaml, .toml:
                append(
                    #"(?m)^[\t ]*(?:-\s*)?[A-Za-z_][\w.-]*(?=\s*[=:])"#, style: .property, source: source, range: range,
                    to: &runs)
            case .html, .xml:
                append(#"</?[A-Za-z][^>]*>"#, style: .keyword, source: source, range: range, to: &runs)
                append(
                    #"\b[A-Za-z_:][-A-Za-z0-9_:.]*(?=\s*=)"#, style: .property, source: source, range: range, to: &runs)
            case .css:
                append(
                    #"(?m)(?:^|[;{])\s*[-A-Za-z]+(?=\s*:)"#, style: .property, source: source, range: range, to: &runs)
