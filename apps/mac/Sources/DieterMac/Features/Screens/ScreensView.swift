        ...
        VStack(spacing: 0) {
            FluidPaneChrome {
                HStack(spacing: 12) {
                    ...
                    .labelsHidden()
                    .frame(width: 190)
                    .disabled(isSessionActive || machines.isEmpty)
                    ...
                    if controller.phase == .streaming {
                        Menu {
                            ...
                            Divider()
                            Toggle("Compose text locally (IME)", isOn: $controller.textInputMode)
    ...
    @ViewBuilder private var primaryAction: some View {
        ...
        default:
            Button("Connect") {
                ...
                controller.connect(machineName: selectedMachine.name) {
                    try await makeConnection(selectedMachine.id)
                }
            }
            .buttonStyle(DieterPrimaryButtonStyle())
