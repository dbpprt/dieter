...
@Test @MainActor func renderMenuBarIconPreview() throws {
    ...
    try writePNG(canvas, scale: 4, to: "/tmp/nauclio-menubar-icon.png")
}
...
@Test @MainActor func renderMenuBarPopoverPreview() throws {
    ...
    let miniHome = NauclioEndpoint(name: "mac-mini", host: "100.121.53.82", port: 4242, daemonID: "d1", online: true)
    let laptop = NauclioEndpoint(name: "macbook-pro", host: "192.168.254.70", port: 4242, daemonID: "d2", online: false, lastSeenAt: isoDate(secondsAgo: 7_200))
    ...
    var boardOne = Nauclio_V1_Board(); boardOne.id = "b1"; boardOne.name = "Agent workspace"
    var boardTwo = Nauclio_V1_Board(); boardTwo.id = "b2"; boardTwo.name = "Main"
    store.state.boards = [boardOne, boardTwo]

    var review = Nauclio_V1_Card()
    ...
    var chat = Nauclio_V1_Card()
    chat.id = "c3"; chat.title = "Standalone chat"; chat.runtime = "running"
    chat.activeSubagents = [Nauclio_V1_Subagent(), Nauclio_V1_Subagent(), Nauclio_V1_Subagent()]
    store.chats = [chat]
    ...
    try writePNG(image, scale: 2, to: "/tmp/nauclio-menubar-popover.png")
}
