...
@Test @MainActor func remoteDesktopHEVCAuthenticatedTransport() async throws {
...
@MainActor private func exerciseRecoveryTransport(codec: String, mode: String) async throws {
    ...
    defer { peer.close() }
    let channel = peer.dataChannel(forLabel: "dieter-session-v2", configuration: RTCDataChannelConfiguration())
