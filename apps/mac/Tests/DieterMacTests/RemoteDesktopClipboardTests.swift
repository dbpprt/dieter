...
@Test func screenClipboardPreservesFilesAndRejectsUnsafeNames() throws {
    ...
    let original = ScreenClipboardContent(items: [.init(kind: 0, name: "empty.txt", mimeType: "text/plain", data: Data()), .init(kind: 0, name: "binary.bin", mimeType: "application/octet-stream", data: Data([0,255,10]))])
    ...
    for name in ["../escape", "/absolute", "..", "a\\b", "nul\0name"] {
        let bad = ScreenClipboardContent(items: [.init(kind: 0, name: name, mimeType: "application/octet-stream", data: Data())])
    ...
    #expect(try FileManager.default.contentsOfDirectory(atPath: root.path).filter { $0.hasPrefix("transfer-") }.count == 8)
