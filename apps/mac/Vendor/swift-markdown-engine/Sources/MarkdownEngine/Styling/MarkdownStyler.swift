...
extension MarkdownStyler {
    ...
    struct StylingContext {
        ...
        var renderedCodeIndexed: [IndexedToken] { classified?.renderedCode ?? Self.indexed(tokens, .codeBlock) }
...
enum MarkdownStyler {
    ...
    static func styleAttributes(
        ...
        // Resolve rendered fences before syntax styling: hidden chart JSON does
        // not need thousands of highlighted spans that will immediately be hidden.
        let renderedCode = styleRenderedCodeBlocks(ctx)
        let renderedCodeStarts = Set(renderedCode.compactMap {
            ($0.attributes[.renderedCodeBlockRange] as? NSValue)?.rangeValue.location
        })
        ...
        result += MarkdownASTStyler.styleAttributes(
            ...
            renderedCodeBlockStarts: renderedCodeStarts, configuration: configuration
        )
