...
public struct NativeTextViewWrapper: NSViewRepresentable {
    ...
    public func updateNSView(_ nsView: NSScrollView, context: Context) {
        ...
        let newRenderedCodeFingerprint = configuration.services.renderedCodeBlocks.fingerprint()
        let renderedCodeServiceChanged = configuration.services.renderedCodeBlocks.renderingDidChangeNotification
            != context.coordinator.configuration.services.renderedCodeBlocks.renderingDidChangeNotification
        let renderedCodeChanged = newRenderedCodeFingerprint != context.coordinator.lastRenderedCodeFingerprint
            || renderedCodeServiceChanged
        if imageChanged || wikiChanged || renderedCodeChanged {
            context.coordinator.lastImageFingerprint = newImageFingerprint
            context.coordinator.lastWikiFingerprint = newWikiFingerprint
            context.coordinator.lastRenderedCodeFingerprint = newRenderedCodeFingerprint
            context.coordinator.configuration.services = configuration.services
            textView.configuration.services = configuration.services
            if imageChanged || wikiChanged || renderedCodeServiceChanged {
                // A new service can change which code languages are rendered.
                if imageChanged || renderedCodeServiceChanged, let tlm = textView.textLayoutManager {
                    tlm.invalidateLayout(for: tlm.documentRange)
                }
                // Restyle live content to preserve edits not yet mirrored into the binding.
                let fullRange = NSRange(location: 0, length: (textView.string as NSString).length)
                if fullRange.length > 0 {
                    context.coordinator.restyleParagraphs([fullRange], in: textView)
                }
            } else if renderedCodeChanged {
                context.coordinator.refreshRenderedCodeBlocks(in: textView)
            }
        }
        textView.isEditable = isEditable
        textView.isSelectable = true
        ...
        if isNodeSwitch {
            ...
            context.coordinator.hasInteractedWithDocument = false
    ...
    public func makeCoordinator() -> Coordinator {
        ...
        coordinator.lastRenderedCodeFingerprint = configuration.services.renderedCodeBlocks.fingerprint()
