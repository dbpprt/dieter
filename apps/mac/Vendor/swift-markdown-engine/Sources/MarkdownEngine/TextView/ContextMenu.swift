...
extension NativeTextViewWrapper.Coordinator {
    ...
    private func replacePreservingAttributes(
        ...
        // Route the attributed replacement through NSTextView so TextKit owns
        // the edit and its undo record, including restoration of attributes.
        let expected = (tv.string as NSString).replacingCharacters(in: range, with: replacement.string)
        tv.insertText(replacement, replacementRange: range)
        return tv.string == expected
    }
