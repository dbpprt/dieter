...
extension NativeTextView {
    ...
    private func restyleTableParagraphsForWidthChange() {
        ...
        coord.refreshRenderedCodeBlocks(in: self)
        var ranges: [NSRange] = []
        var seen: Set<String> = []
        let fullRange = NSRange(location: 0, length: storage.length)
        storage.enumerateAttribute(.scrollableBlockFullRange, in: fullRange, options: []) { value, anchor, _ in
            guard storage.attribute(.renderedCodeBlockSource, at: anchor.location, effectiveRange: nil) == nil,
                  let v = value as? NSValue else { return }
            let r = v.rangeValue
