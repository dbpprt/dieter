...
extension NativeTextViewCoordinator {
    ...
    /// TextKit can replay its attributed-text undo without delivering
    /// `textDidChange`. Publish that restored buffer through the usual Markdown
    /// pipeline, scoped to the manager belonging to the displayed document.
    @objc func synchronizeCompletedUndo(_ notification: Notification) {
        guard !isWritingToolsActive,
              let manager = notification.object as? UndoManager,
              undoManagers[documentId ?? "__default__"] === manager,
              let textView else { return }
        let restored = configuration.rawSourceMode
            ? textView.string
            : WikiLinkService.makeStorageState(
                from: textView.string, existingMetadata: wikiLinkMetadata,
                textStorage: textView.textStorage).storage
        // lastComputedStorage advances synchronously, unlike the deferred
        // binding write. This also handles Undo before the previous edit's
        // binding callback lands, and skips an already-delivered text change.
        guard restored != lastComputedStorage else { return }
    ...
    public func textDidChange(_ notification: Notification) {
        ...
        if configuration.rawSourceMode {
            ...
            let storageChanged = lastComputedStorage != tv.string
            lastComputedStorage = tv.string
            if storageChanged || tv.string != lastSyncedText {
        ...
        if !wtActive {
            ...
            let storageChanged = storageState.storage != self.lastComputedStorage
...
#endif
            if storageChanged || storageState.storage != self.lastSyncedText {
                DispatchQueue.main.async {
