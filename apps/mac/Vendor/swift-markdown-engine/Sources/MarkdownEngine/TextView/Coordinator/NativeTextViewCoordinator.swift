...
public final class NativeTextViewCoordinator: NSObject, NSTextViewDelegate {
    ...
    var configuration: MarkdownEditorConfiguration = .default {
        didSet {
            ...
            subscribeToRenderedCodeNotification()
    ...
    var lastRenderedCodeFingerprint: AnyHashable?
    var hasInteractedWithDocument = false
    private var renderedCodeObserver: NSObjectProtocol?
    private var registeredRenderedCodeObserverName: Notification.Name?
    ...
    var onURLClick: (@MainActor (URL) -> Bool)?
    ...
    init(text: Binding<String>,
        ...
        for name in [Notification.Name.NSUndoManagerDidUndoChange, .NSUndoManagerDidRedoChange] {
            NotificationCenter.default.addObserver(
                self, selector: #selector(synchronizeCompletedUndo(_:)), name: name, object: nil)
        }
    ...
    private func subscribeToRenderedCodeNotification() {
        let name = configuration.services.renderedCodeBlocks.renderingDidChangeNotification
        guard name != registeredRenderedCodeObserverName else { return }
        if let renderedCodeObserver { NotificationCenter.default.removeObserver(renderedCodeObserver) }
        renderedCodeObserver = nil
        registeredRenderedCodeObserverName = name
        guard let name else { return }
        renderedCodeObserver = NotificationCenter.default.addObserver(
            forName: name, object: nil, queue: .main
        ) { [weak self] _ in
            guard let self, let tv = self.textView else { return }
            self.lastRenderedCodeFingerprint = self.configuration.services.renderedCodeBlocks.fingerprint()
            self.refreshRenderedCodeBlocks(in: tv)
        }
    }

    func beginNativeInteraction() {
        guard !hasInteractedWithDocument else { return }
        hasInteractedWithDocument = true
        activeTokenMemo = nil
    }
