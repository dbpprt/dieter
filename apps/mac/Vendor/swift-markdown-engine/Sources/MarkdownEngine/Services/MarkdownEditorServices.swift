...
// MARK: - Rendered Fenced Code

/// Supplies presentation for fenced code such as Mermaid or Vega diagrams.
/// Native editor calls are synchronous on the main thread. Asynchronous renderers
/// should return a cached image or placeholder, then post their notification on
/// the main thread once an image is available. Return nil for ordinary code.
public protocol RenderedCodeBlockRenderer: Sendable {
    func render(code: String, language: String, availableWidth: CGFloat, theme: MarkdownEditorTheme) -> RenderedCodeBlockResult?
    func fingerprint() -> AnyHashable
    var renderingDidChangeNotification: Notification.Name? { get }
}

public extension RenderedCodeBlockRenderer {
    func fingerprint() -> AnyHashable { 0 }
    var renderingDidChangeNotification: Notification.Name? { nil }
}

public struct RenderedCodeBlockResult: Sendable {
    public let image: NSImage
    public let size: CGSize
...
public struct NoOpRenderedCodeBlockRenderer: RenderedCodeBlockRenderer {
    public init() {}
    public func render(code: String, language: String, availableWidth: CGFloat, theme: MarkdownEditorTheme) -> RenderedCodeBlockResult? { nil }
}
...
public struct MarkdownEditorServices: Sendable {
    ...
    public var renderedCodeBlocks: any RenderedCodeBlockRenderer
    ...
    public init(
        ...
        renderedCodeBlocks: any RenderedCodeBlockRenderer = NoOpRenderedCodeBlockRenderer(),
        ...
        self.latex = latex
        self.renderedCodeBlocks = renderedCodeBlocks
        self.bus = bus
    }
