...
fun AppUpdateDialog(manager: AppUpdateManager) {
    ...
    when (val current = state) {
        is AppUpdateState.Available -> AlertDialog(
            ...
            text = {
                UpdateDialogText(
                    title = "Dieter ${current.release.version}",
            ...
            AlertDialog(
                ...
                text = {
                    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        Text("${(progress * 100).toInt()}%", color = DieterMuted)
                    }
                },
                confirmButton = {},
            )
        ...
        is AppUpdateState.ReadyToInstall -> AlertDialog(
            ...
            text = {
                UpdateDialogText(
                    title = "Dieter ${current.release.version} is verified.",
                    detail = if (current.installationAllowed) {
                        "Android will show the final installation confirmation."
                    } else {
                        "Allow Dieter to install updates, then return here and tap Install."
                    },
                )
            },
            confirmButton = {
...
private fun UpdateDialogText(title: String, detail: String) {
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Text(detail, color = DieterMuted, style = MaterialTheme.typography.bodyMedium)
    }
}
