package com.dbpprt.nauclio.widget
...
import com.dbpprt.nauclio.connection.NauclioConnectionState
 ...
 * pull a fresh frame.
 */
class NauclioActivityWidgetProvider : AppWidgetProvider() {
    ...
    override fun onDeleted(context: Context, appWidgetIds: IntArray) {
        NauclioWidgetPrefs.delete(context, appWidgetIds)
    }
    ...
    companion object {
        const val ACTION_REFRESH = "com.dbpprt.nauclio.widget.REFRESH"
        private const val COMPACT_MAX_WIDTH_DP = 200

        fun updateAll(context: Context) {
            val manager = AppWidgetManager.getInstance(context) ?: return
            val ids = manager.getAppWidgetIds(ComponentName(context, NauclioActivityWidgetProvider::class.java))
            ids.forEach { render(context, manager, it) }
        }

        fun render(context: Context, manager: AppWidgetManager, appWidgetId: Int) {
            val config = NauclioWidgetPrefs.config(context, appWidgetId)
            ...
            val lastSyncAtMs = NauclioWidgetPrefs.lastSyncAtMs(context)
            ...
            val adapter = Intent(context, NauclioWidgetService::class.java)
                .putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId)
                .setData(Uri.parse("nauclio-widget://list/$appWidgetId"))
        ...
        internal fun connectionState(context: Context): NauclioConnectionState =
            (context.applicationContext as NauclioApplication).container.connectionManager.state.value

        internal fun hostname(context: Context, state: NauclioConnectionState): String? =
            state.projectHosts.values.firstOrNull { it.online }?.hostname
                ...
                ?: NauclioSyncStore(context.applicationContext)
        ...
        private fun refreshIntent(context: Context): PendingIntent = PendingIntent.getBroadcast(
            context,
            1,
            Intent(context, NauclioActivityWidgetProvider::class.java).setAction(ACTION_REFRESH),
