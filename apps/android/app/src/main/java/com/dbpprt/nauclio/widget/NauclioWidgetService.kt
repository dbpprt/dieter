...
class NauclioWidgetService : RemoteViewsService() {
...
internal class ActivityRemoteViewsFactory(
    ...
    override fun onDataSetChanged() {
        ...
        compact = NauclioActivityWidgetProvider.isCompact(config.style, options)
        val state = NauclioActivityWidgetProvider.connectionState(context)
        rows = buildWidgetModel(
            cards = state.cards,
            chats = state.chats,
            conversations = state.activeConversations,
            projects = state.projects,
            hostname = NauclioActivityWidgetProvider.hostname(context, state),
            lastSyncAtMs = NauclioWidgetPrefs.lastSyncAtMs(context),
            config = config,
            compact = compact,
        ).rows
    }
    ...
    private fun itemView(row: WidgetRow.Item): RemoteViews {
        ...
        views.setOnClickFillInIntent(
            R.id.widget_row_root,
            Intent().putExtra(NauclioSyncService.EXTRA_CARD_ID, row.cardId),
        )
        return views
    }
