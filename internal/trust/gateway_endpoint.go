...
func VerifyGatewayEndpoint(public ed25519.PublicKey, token, issuer, subject, contract string, now time.Time) (GatewayEndpointClaims, error) {
	...
	if err != nil || origin != claims.Endpoint || (strings.HasPrefix(issuer, "https:") && !strings.HasPrefix(origin, "https:")) || claims.Issuer != issuer || claims.Subject != subject ||
		!validGitHubSubject(subject) || claims.Audience != "dieter-gateway-endpoint" || claims.Contract != contract ||
