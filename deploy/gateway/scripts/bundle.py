...
import time
import subprocess
from common import ROOT, IMAGE, IDENTITY, ISSUER, NAME, atomic, digest, keys, read_json, require, run, semantic_version, validate_compatibility_policy
...
def pack(output, revision, version, image, built_at, probes=None):
    ...
    policy = validate_compatibility_policy(read_json(ROOT / "compatibility-policy.json"), version)
    out = Path(output)
    ...
    manifest = {"manifestVersion": 1, "bundleInterfaceVersion": 1, "releaseVersion": version,
                ...
                "platforms": ["linux/amd64", "linux/arm64"], "compatibilityPolicy": policy, "gatewayStoreSchema": 1,
    ...
    atomic(out / (ARCHIVE + ".version"), version + "\n", 0o644)
    return manifest


def validate_manifest(m):
    keys(m, "manifestVersion bundleInterfaceVersion releaseVersion sourceRevision builtAt image platforms compatibilityPolicy gatewayStoreSchema bundle dependencies".split(), "manifest")
    ...
    semantic_version(m["releaseVersion"])
    validate_compatibility_policy(m["compatibilityPolicy"], m["releaseVersion"])
    require(re.fullmatch(r"[a-f0-9]{40}", m["sourceRevision"]), "invalid release identity")
